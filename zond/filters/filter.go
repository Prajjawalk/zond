...
func (f *Filter) blockLogs(ctx context.Context, header *protos.BlockHeader) (logs []*types.Log, err error) {
	if bloomFilter(types.BytesToBloom(header.TxBloom), f.addresses, f.topics) {
		found, err := f.checkMatches(ctx, header)
		if err != nil {
			return logs, err
		}
